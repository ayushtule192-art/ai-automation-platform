import { getOpenAIClient, CHAT_MODEL } from "../../agents/chat/openai.client.js";
...
export class CallingService {
  ...
  isConfigured(): boolean {
    return isAiProviderConfigured("twilio") && isAiProviderConfigured("openai");
  }
  ...
  async scheduleContacts(
    ...
    if (!this.isConfigured()) {
      throw AppError.badRequest("Twilio and OpenAI must be configured", "CALLING_NOT_CONFIGURED");
    }
  ...
  private async generateCallResponse(callId: string, userSpeech: string): Promise<string> {
    ...
    const completion = await openai.chat.completions.create({
      model: CHAT_MODEL,
      messages: [
        { role: "system", content: CALLING_SYSTEM_PROMPT },
        ...history.map((h) => ({
